...
import PoolGame.ObserverPattern.Observer;
...
public class Ball implements Drawable, Movable, Subject {
    ...
    /**
     * @param source The instance of the Ball itself (for Prototype)
     */
    public Ball(Ball source) {
        this.shape = new Circle(source.originalPos[0], source.originalPos[1], RADIUS);
        this.mouseDragLine = new Line();
        this.handle = new Circle();
        this.handle.setVisible(false);
        this.mouseDragLine.setVisible(false);

        this.setXPos(source.getXPos());
        this.setYPos(source.getYPos());
        this.setXVel(source.getXVel());
        this.setYVel(source.getYVel());
        this.vel[0] = source.getXVel();
        this.vel[1] = source.getYVel();
        this.setMass(source.getMass());
        this.setBallType(source.getBallType());
        this.setPocketAction(source.getPocketAction());
        colourStr = source.getColour();
    }
    ...
    /**
     * Sets ball fall counter.
     * @param fallCounter The no. of times the ball has fell into pocket.
     */
    ...
    /**
     * @param disabled Set boolean disabled.
     */
    ...
    /**
     * @return The name of the colour of the ball. 
     */
    ...
    /** Enables the ball, makes it visible and sets disabled to false */
    public void enable() {
    ...
    private void registerCueBallMouseAction() {
        ...
        this.shape.setOnMouseClicked(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    ...
                    this.mouseDragLine.setEndX(this.shape.getCenterX()+30);
                    this.mouseDragLine.setEndY(this.shape.getCenterY()+30);

                    this.mouseDragLine.setStrokeWidth(5);
                    this.handle.setVisible(true);
                    this.handle.setRadius(10);
                    this.handle.setCenterX(this.shape.getCenterX()+30);
                    this.handle.setCenterY(this.shape.getCenterY()+30);
                    // this.handle.setFill(Color.BLUEVIOLET);
                    this.handle.setFill(Color.MEDIUMVIOLETRED);
                    this.mouseDragLine.setStroke(Color.MAROON);
                }
            }
        );

        this.handle.setOnMouseDragged(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    this.handle.setVisible(true);
                    this.handle.setRadius(10);
                    this.handle.setCenterX(actionEvent.getSceneX());
                    this.handle.setCenterY(actionEvent.getSceneY());
        ...
        this.handle.setOnMouseReleased(
            (actionEvent) -> {
                if (this.hasStopped()) {
                    this.handle.setRadius(10);
                    this.handle.setCenterX(this.shape.getCenterX()+60);
                    this.handle.setCenterY(this.shape.getCenterY()+60);
    ...
    */
    ...
    /**
     * Alert/Updates the game score.
     * @param game The game instance.
     */
    public void alert(Game game) {
        this.ballObserver.update(game);
    }

    /**
     * Attach the ball observer to the ball.
     * @param observer The BallObserver instance. 
     */
    @Override
    public void attach(Observer observer) {
        // ballObserver.add(observer); 
        this.ballObserver = observer;
    }

    /** 
     * Clones the ball.
     * @return Returns the Cloned Ball.
    */
    public Ball clone() {
